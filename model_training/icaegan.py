def train_icaegan(exp_num, n_epochs, input_trainset, print_samp = True):    
    # -*- coding: utf-8 -*-
    """ICAEGAN_cifar_final.ipynb

    Automatically generated by Colaboratory.

    Original file is located at
        https://colab.research.google.com/drive/1C2LZHt8_UBZKiwemr1hymNvJElgEKXxP
    """
    ...
    class Inverse_Autoencoder(nn.Module):
        ...
        def encode(self, x):
            ...
            c = nn.Softmax(dim=1)(self.conv5c(x)) # this is softmax for CLASSIFICATION. Shapes3d is not 1-classif..
    ...
    def print_g_sample():
        with torch.no_grad():
            ...
            generated = .5*(IAE.forward(varis, codes).cpu() + 1)
    ...
    IAE_optimizer = optim.Adam(IAE.parameters(),
                             lr = learning_rate,
                             betas = (beta_1, beta_2))
    ...
    for epoch in range(1, n_epochs+1):
        ...
        for X, code in train_loader:
            ...
            X_fake = IAE.forward(rand_v, rand_c)

            #print(X_fake.shape, rand_c.shape)
            ...
            vhat, chat = IAE.pass_thru(rand_v, rand_c)
            v_loss = MSE_loss(vhat, rand_v)
            c_loss = BCE_loss(chat, rand_c)

            ## Loss combination
            model_loss = G_loss + v_loss + c_loss
            model_loss.backward()
            IAE_optimizer.step()

            D_losses.append(D_loss.data.item())
            G_losses.append(G_loss.data.item())
            v_losses.append(v_loss.data.item())
        ...
        if print_samp == True:
	        if epoch % print_stride == 0:
	            print('Epoch {} - loss_ae (mse): {:.3f}, loss_cond: {:.3f}, loss_D: {:.3f}, loss_G: {:.3f}'.format((epoch),
	                                                                   torch.mean(torch.FloatTensor(v_losses)),
	                                                                   torch.mean(torch.FloatTensor(c_losses)),
	                                                                   torch.mean(torch.FloatTensor(D_losses)),
	                                                                   torch.mean(torch.FloatTensor(G_losses))))

	            v_loss_tracker.append(torch.mean(torch.FloatTensor(v_losses)))
    ...
    acc = test_acc()
    torch.save(IAE.state_dict(), f'icaegan_cifar_half_{exp_num}_{n_epochs}_G.pt')
    return D, IAE, acc