def train_caegan(exp_num, n_epochs, input_trainset, print_samp = True):
    ...
    for epoch in range(1, n_epochs+1):
        ...
        for X, code in train_loader:
            ...
            v, c = AE.encode(X)
            condition_loss = BCE_loss(c.view(mini_batch, c_dim), code)



            ## Loss combination
            model_loss = G_loss + recon_loss + condition_loss #+ cyc_loss #condition is c-cycle and c-encode. maybe dont even need condition loss then..
            model_loss.backward()
            AE_optimizer.step()
    ...
    acc = test_acc()
    torch.save(AE.state_dict(), f'self_test_caegan_cifar_{exp_num}_5p_G.pt')
    return D, AE, acc
